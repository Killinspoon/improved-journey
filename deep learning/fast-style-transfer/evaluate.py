...
import subprocess
import numpy
import videobuilder as vb#import buildVideo
...
def ffwd(data_in, paths_out, checkpoint_dir, device_t='/gpu:0', batch_size=4):
    ...
    if is_paths:
        ...
        assert len(data_in[0]) == len(paths_out)
        img_shape = X[0].shape
...
def main():
    ...
    checkpoint_name = 'wave' # la_muse,rain_princess,scream,udnie,wave,wreck
    checkpoint_dir = 'D:\\Python\\fast-style-transfer\\test_checkpoint\\' + checkpoint_name + '.ckpt'
    default_device = DEVICE
    default_batch = BATCH_SIZE


    in_path = 'D:\\Python\\video2frames\\Not_the_Bees'
    out_path = 'D:\\\Python\\fast-style-transfer\\Not_the_Bees_' + checkpoint_name
    ...
    ffwd(full_in, full_out, checkpoint_dir, device_t=default_device,
                    batch_size=default_batch)

    filename = 'Not_the_Bees'
    vb.buildvideo(checkpoint_name, out_path, filename, fps)
    #
    # ffwd_to_img(full_in, full_out, checkpoint_dir,
    #             device='/gpu:0')
